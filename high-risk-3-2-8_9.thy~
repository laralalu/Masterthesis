...
abbreviation "A1 \<equiv> \<lfloor>\<^bold>\<forall>x. (high_risk x) \<^bold>\<and> (provider_of x d) \<^bold>\<rightarrow> \<^bold>\<circle>d<(stit d (comply_with_req_chap_2 x))>\<rfloor>"
...
abbreviation "A4 \<equiv> \<lfloor>\<^bold>\<forall>x. (high_risk x) \<^bold>\<and> (provider_of x d) \<^bold>\<rightarrow> \<^bold>\<circle>d<(stit d (rms_estab_impl_maint x))>\<rfloor>"
abbreviation "A5 \<equiv> \<lfloor>\<^bold>\<forall>x. (high_risk x) \<^bold>\<and> (provider_of x d) \<^bold>\<rightarrow> (\<^bold>\<circle>d<(stit d (rms_iterative_process rms_of_sys x)) \<^bold>\<and> (stit d (rms_regular_updates rms_of_sys x))>)\<rfloor>"
abbreviation "A6 \<equiv> \<lfloor>\<^bold>\<forall>x. (high_risk x) \<^bold>\<and> (provider_of x d) \<^bold>\<rightarrow> (\<^bold>\<circle>d<(stit d (rms_con_ident_and_analysis rms_of_sys x)) \<^bold>\<and> (stit d (rms_con_est_and_eval rms_of_sys x))
  \<^bold>\<and> (stit d (rms_con_eval_poss_risks rms_of_sys x)) \<^bold>\<and> (stit d (rms_con_adapt_man_measures rms_of_sys x))>)\<rfloor>"
abbreviation "A7 \<equiv> \<lfloor>\<^bold>\<forall>x. (high_risk x) \<^bold>\<and> (provider_of x d) \<^bold>\<rightarrow> (\<^bold>\<circle>d<(stit d (taken_into_account''' rms_sys x consequences_sys x)) \<^bold>\<and> (stit d (taken_into_account'' rms_sys x soa_sys x))>)\<rfloor>"
abbreviation "A8 \<equiv> \<lfloor>\<^bold>\<forall>x. (high_risk x) \<^bold>\<and> (provider_of x d) \<^bold>\<rightarrow> ((used_in_acc_wi_purpose purpose_sys x) \<^bold>\<or> (used_foreseeable_misuse x) \<^bold>\<rightarrow>
  \<^bold>\<circle>d<stit d (residual_risk_acceptable risk_measures_establ_2d residual_risks x)>)\<rfloor>"
abbreviation "A9 \<equiv>  \<lfloor>\<^bold>\<forall>x. (high_risk x) \<^bold>\<and> (provider_of x d) \<^bold>\<rightarrow> \<^bold>\<circle>d<(stit d (communicate_to_user residual_risks x))>\<rfloor>"
abbreviation "A10 \<equiv>  \<lfloor>\<^bold>\<forall>x. (high_risk x) \<^bold>\<and> (provider_of x d) \<^bold>\<rightarrow> (\<^bold>\<circle>d<(stit d (ensure_elem_reduction_risks risks x rms_sys x)) \<^bold>\<and> 
...
(*k=credit institutions, would need to be added to DDL*)
...
abbreviation "theory \<equiv>  A1 \<and> A1a \<and> A2 \<and> A3 \<and> A4 \<and> A5 \<and> A6 \<and> A7 \<and> A8 \<and> A9 \<and> A10 \<and> A11 \<and> A12 \<and> A13 \<and> A14"
