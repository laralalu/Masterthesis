...
axiomatization where 
  D: "\<lfloor>\<^bold>\<not> ((OB \<phi>) \<^bold>\<and> (OB (\<^bold>\<not> \<phi>)))\<rfloor>" (*Axiom D: seriality of R.*)
...
(*axioms test*)
lemma OBK: "\<lfloor>(OB (p \<^bold>\<rightarrow> q)) \<^bold>\<rightarrow> (OB p \<^bold>\<rightarrow> OB q )\<rfloor>" by simp
lemma OBNC: "\<lfloor>(OB p) \<^bold>\<rightarrow> \<^bold>\<not> (OB (\<^bold>\<not> p))\<rfloor>" by (simp add: seriality)
lemma OBMP: "\<lbrakk>\<lfloor>OB A\<rfloor>; \<lfloor>(OB A) \<^bold>\<rightarrow> B\<rfloor>\<rbrakk> \<Longrightarrow> \<lfloor>B\<rfloor>" by simp
lemma OBNEC: "\<lfloor>A\<rfloor> \<Longrightarrow> \<lfloor>OB A\<rfloor>" by simp
