...
consts (*Predicates/relations*)
   ...
   delegated_acts_adapted::"ag\<Rightarrow>\<sigma>" (*delegated acts are adapted by eu-commission*)
...
abbreviation "A3 \<equiv> \<lfloor>\<^bold>\<forall>x. (((is_used_areas_1to8_Annex_3 x) \<^bold>\<and> (poses_risk_equivalent_greater_systems_Annex_3 x)) 
                   \<^bold>\<rightarrow> (\<^bold>\<not>(\<^bold>\<circle>c<\<^bold>\<not> stit c (delegated_acts_adapted c)>)))\<rfloor>"
