...
abbreviation "A7 \<equiv> \<lfloor>\<^bold>\<forall>x::aiSys. \<^bold>\<not>(compliance_req_chap2 x) \<^bold>\<rightarrow> \<^bold>\<circle>d<stit d (corrective_action_taken x)>\<rfloor>"
abbreviation "A8 \<equiv> \<lfloor>\<^bold>\<forall>x::aiSys. \<^bold>\<not>(compliance_req_chap2 x) \<^bold>\<rightarrow> \<^bold>\<circle>d<stit d (inform_com_auth x)>\<rfloor>" 
abbreviation "A9 \<equiv> \<lfloor>\<^bold>\<forall>x::aiSys. (\<^bold>\<not>(compliance_req_chap2 x) \<^bold>\<and> nat_body_involved) \<^bold>\<rightarrow> \<^bold>\<circle>d<stit d (inform_nat_body x)>\<rfloor>"
abbreviation "A10 \<equiv> \<lfloor>\<^bold>\<forall>x::aiSys. \<^bold>\<circle>d<stit d (CE_marking_art_49_affixed x)>\<rfloor>"
abbreviation "A11 \<equiv> \<lfloor>\<^bold>\<forall>x::aiSys. (demo_requested_by_nat_comp_auth) \<^bold>\<rightarrow> \<^bold>\<circle>d<stit d (conformance_art_2_demonstrated x)>\<rfloor>"
...
AX: "\<lfloor>\<^bold>\<forall>x::aiSys. \<^bold>\<circle>d<(stit d (compliance_req_chap2 x)) \<^bold>\<and> (on_market x) \<^bold>\<rightarrow> (\<^bold>\<not>(stit d (inform_com_auth x)))>\<rfloor>" and
